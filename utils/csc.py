...
def main():
    ...
    for rec in pixpat.Rec:
        for color_range in pixpat.Range:
            print(f'Format: {fmt} Encoding: {rec}, Range: {color_range}')

            fb = kms.DumbFramebuffer(card, width, height, fmt)

            fb.begin_cpu_access('w')
            kms.testpat.draw_test_pattern(
                fb, pattern=args.pattern, rec=rec, color_range=color_range
            )
            fb.end_cpu_access()
            ...
            req.add_plane(
                ...
                params={enc_prop_id: rec.value, range_prop_id: color_range.value},
            )
