...
def main():
    ...
    buf = pixpat.Buffer(
        planes=planes, fmt=args.format, width=args.width, height=args.height, strides=strides
    )

    params = dict(args.params) if args.params else None

    try:
        pixpat.draw_pattern(buf, args.pattern, rec=rec, color_range=color_range, params=params)
    ...
    if format_obj.color == PixelColorEncoding.YUV:
        filename = generate_filename(
            args.width, args.height, args.format, args.rec_standard, args.full_range
        )
    else:
