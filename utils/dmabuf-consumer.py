...
def recv_ints(sock, num_ints):
    data, _ancdata, _flags, _addr = sock.recvmsg(struct.calcsize(f'{num_ints}i'))
    if not data:
...
def main():
    ...
    for src_id in range(num_sources):
        ...
        for i in range(num_fbs):
            data, fds, _flags, _addr = socket.recv_fds(client_sock, struct.calcsize('iiiiiii'), 1)
            fb_id, _xxx_fd, fb_size, fb_format, width, height, _stride = struct.unpack('iiiiiii', data)
            fd = fds[0]
