...
class Overlay:
    def __init__(self, plane: kms.Plane) -> None:
        self.plane = plane
        self.fb = kms.DumbFramebuffer(
            plane.card, PLANE_WIDTH, PLANE_HEIGHT, kms.PixelFormats.XRGB8888
        )
...
def main():
    ...
    req.commit_sync(allow_modeset=True)
    print('Planes on Screen0')
    sys.stdin.readline()

    req = kms.AtomicReq(card)
    ovls[1].disable(req)
    req.commit_sync(allow_modeset=True)
    print('Plane1 disabled')
    sys.stdin.readline()

    req = kms.AtomicReq(card)
    ovls[1].setup(req, screen1)
    req.commit_sync(allow_modeset=True)
    print('Plane1 on Screen1')
    sys.stdin.readline()

    req = kms.AtomicReq(card)
    ovls[0].disable(req)
    req.commit_sync(allow_modeset=True)
    print('Plane0 disabled')
    sys.stdin.readline()

    req = kms.AtomicReq(card)
    ovls[0].setup(req, screen1)
    req.commit_sync(allow_modeset=True)
    print('Plane0 on Screen1')
    sys.stdin.readline()
